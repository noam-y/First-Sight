...
def category_search():
    if request.method == 'POST':
        ...
        include_adult = request.form.get('agelimit') is not None
...
def sort_one_movie(movie, single=False):
    if 'title' in movie:
        try:
            json_movie_details = {'title': movie["title"], 'overview': movie['overview'],
                                  'poster': 'http://image.tmdb.org/t/p/w500' + movie["poster_path"], 'id': movie["id"]}
        except(TypeError):
            json_movie_details = {
                'title': movie["title"], 'overview': movie['overview'], 'poster': QUESTION_MARK, 'id': movie["id"]}

    else:
        try:
            json_movie_details = {'title': movie["name"], 'overview': movie['overview'],
                                  'poster': 'http://image.tmdb.org/t/p/w500' + movie["poster_path"], 'id': movie["id"]}
        except(TypeError):
            json_movie_details = {
                'title': movie["name"], 'overview': movie['overview'], 'poster': QUESTION_MARK, 'id': movie["id"]}
